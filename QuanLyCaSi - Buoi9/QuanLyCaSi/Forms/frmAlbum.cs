...
namespace QuanLyCaSi.Forms
{
    public partial class frmAlbum : Form
    {
        ...
        private void LoadListBaiHat(string excludeMaAlbum = null)
        {
            dsBaiHat.Clear();
            lBH.Items.Clear();
            ...
            {
                conn.Open();
                string sql = @"SELECT MaBH, TenBH FROM dbo.Single
                               WHERE MaBH NOT IN (
                                   SELECT MaBH FROM dbo.AlbumSingle
                                   WHERE (@ExcludeMa IS NULL OR MaAlbum <> @ExcludeMa)
                               )
                               ORDER BY TenBH";
                using (SqlCommand cmd = new SqlCommand(sql, conn))
                {
                    cmd.Parameters.AddWithValue("@ExcludeMa", (object)excludeMaAlbum ?? DBNull.Value);
        ...
        private void ClearForm()
        {
            ...
            cboCS.SelectedIndex = -1;
            // Reload lại ListBox bài hát chưa có album
            LoadListBaiHat();
        }
        ...
        private void AnCotGrid()
        {
            ...
            if (dataGridView1.Columns.Contains("DanhSachBaiHat")) dataGridView1.Columns["DanhSachBaiHat"].Width = 400;
        }
